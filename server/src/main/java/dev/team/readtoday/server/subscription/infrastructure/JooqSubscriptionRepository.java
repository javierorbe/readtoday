...
public class JooqSubscriptionRepository implements SubscriptionRepository {
  ...
  public Optional<List<Subscription>> getAllByUserId(UserId userId) {
    ...
    dsl.select(SUBSCRIPTION.USER_ID, SUBSCRIPTION.CHANNEL_ID)
        .from(SUBSCRIPTION)
        .where(SUBSCRIPTION.USER_ID.eq(userId.toString()))
        .fetch();
  ...
  public void remove(Subscription subscription) {
    dsl.deleteFrom(SUBSCRIPTION)
       .where(SUBSCRIPTION.USER_ID.eq(subscription.getIdUser().toString())
       .and(SUBSCRIPTION.CHANNEL_ID.eq(subscription.getIdChannel().toString()))
       ).execute();
